...
Saves 5 publication-quality figures to reports/plots/:
...
def main():
    ...
    print("\n[1/5]  Surfaces overview ...")
    plot_01_surfaces_overview()

    print("\n[2/5]  Generating training data + fitting all models ...")
    all_results = []
    ...
    print("\n[5/5]  Identifier convergence simulation ...")
    plot_05_identifier_convergence()

    print("\n" + "=" * 60)
    print(f"  All 5 plots saved to:  {SAVE_DIR.resolve()}")
    print("=" * 60)

    # ── Smoke-test summary ───────────────────────────────────────────────
    saved = sorted(SAVE_DIR.glob("*.png"))
    assert len(saved) >= 5, f"Expected 5 plots, found {len(saved)}"
    for p in saved:
        sz = p.stat().st_size
